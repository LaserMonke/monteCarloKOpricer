...
with st.sidebar:
    ...
    live = st.toggle("Use live spot", value=HAS_YF,
                     disabled=not HAS_YF,
                     help="Pulls the latest traded price via yfinance "
                          "(15-20 min delayed). Install: pip install yfinance")
    live_px, live_src = None, None
    if live:
        try:
            live_px, live_src = fetch_spot(ticker.strip().upper())
            st.success(f"{ticker.strip().upper()} spot: "
                       f"**{live_px:,.2f}** · {live_src}")
    ...
    r = st.slider("Risk-free rate r — drift under ℚ, used for PRICING",
                  0.00, 0.15, 0.01, 0.0025)
    mu = st.slider("Real-world drift μ — diagnostics only, NOT pricing",
                   -0.50, 0.50, 0.01, 0.01,
                   help="By no-arbitrage, μ drops out of the price "
                        "(risk-neutral valuation). It only affects "
                        "real-world statistics such as the P-measure "
                        "knock-out probability shown below.")
...
if abs(mu - (r - q)) < 1e-12:
    ko_p = res["ko_prob"]                      # measures coincide
else:
    ko_p = ko_prob_real_world(c, mu, min(n_sims, 250_000), int(seed), bridge)
...
m2.metric("Closed-form benchmark", f"{benchmark:.4f}",
          ...
          delta_color="off")
m3.metric("KO probability (ℚ, r-drift)", f"{res['ko_prob']:.2%}",
          f"real-world ℙ (μ={mu:.0%}): {ko_p:.2%}", delta_color="off")
...
payoff. Your inputs μ = r = 1% happen to coincide, so nothing visibly
changes here, but crank μ to 30% and the price stays put — only the
*real-world* knock-out probability (shown alongside the ℚ one above) moves.
