...
def cached_price(s0, k, b, sig, r, q, days, n, seed, bridge, cv):
...
@st.cache_data(show_spinner=False, max_entries=6)
def cached_ko_p(s0, k, b, sig, r, q, days, mu, n, seed, bridge):
    return ko_prob_real_world(Contract(s0, k, b, sig, r, q, days),
                              mu, n, seed, bridge)
...
with st.sidebar:
    ...
    with st.form("pricing_form", border=False):
        ...
        sigma = st.number_input(
            "Volatility σ (annual)" + (" · from ATM IV" if iv_val else ""),
            0.01, 5.00, float(iv_val) if iv_val else 0.80, 0.01,
            format="%.4f", key=f"sig_{'mkt' if iv_val else 'man'}")
        r = st.number_input("Risk-free rate r — pricing drift under ℚ",
                            0.00, 0.25, 0.01, 0.0025, format="%.4f")
        mu = st.number_input(
            "Real-world drift μ — diagnostics only"
            + (" · from 1y history" if mu_hist is not None else ""),
            -1.00, 2.00, float(mu_hist) if mu_hist is not None else 0.01,
            0.01, format="%.4f",
            key=f"mu_{'mkt' if mu_hist is not None else 'man'}",
            help="By no-arbitrage, μ never enters the price — it only moves "
                 "the real-world (ℙ) knock-out probability shown on the "
                 "cards.")
        q = st.number_input("Dividend yield q", 0.00, 0.20, 0.00, 0.0025,
                            format="%.4f")
...
ko_p = (res["ko_prob"] if abs(mu - (r - q)) < 1e-12 else
        cached_ko_p(s0, k, barrier, sigma, r, q, int(days), mu,
                    min(int(n_sims), 250_000), int(seed), bridge))
...
<div class="ticket">
  ...
  <div class="name">Up-and-Out Call, <em>{res['price']:.4f}</em> mid</div>
  <div class="terms">
...
st.markdown(f"""
<div class="cards">
  <div class="card hero"><div class="k">Monte Carlo price</div>
    <div class="v">{res['price']:.4f}</div>
    <div class="s">95% CI <b>[{ci_lo:.4f}, {ci_hi:.4f}]</b></div></div>
  ...
  <div class="card"><div class="k">KO probability</div>
    ...
    <div class="s">ℚ measure · ℙ (μ={mu:.0%}): <b>{ko_p:.1%}</b></div></div>
  ...
  <div class="card"><div class="k">Std error</div>
...
knock-out probability on the ticket moves.
...
""")
    summary = {
        ...
        "results": {"mc_price": res["price"], "std_error": res["se"],
