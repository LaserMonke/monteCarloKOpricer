...
with st.sidebar:
    ...
    restrike = st.toggle("Re-strike contract to spot", value=False,
                         disabled=live_px is None,
                         help="For pricing other stocks: sets K = live spot "
                              "(ATM) and B = K × 250/150, preserving the "
                              "original SPCX moneyness. Off = fixed 150/250.")
    k_def = (float(round(live_px, 2)) if (restrike and live_px) else 150.0)
    b_def = float(round(k_def * 250.0 / 150.0, 2))
    ...
    with st.form("pricing_form", border=False):
        s0 = st.number_input("Spot S₀" + (" (override)" if live_px else ""),
                             ...
                             0.5, format="%.2f")
        st.header("Contract")
        k = st.number_input("Strike K", 1.0, 100_000.0, k_def, 1.0,
                            key=f"k_{'rs' if restrike and live_px else 'def'}")
        barrier = st.number_input("Knock-out barrier B", 1.0, 200_000.0,
                                  b_def, 1.0,
                                  key=f"b_{'rs' if restrike and live_px else 'def'}")
        ...
        r_pct = st.number_input("Risk-free rate r (%)",
                                0.0, 25.0, 1.0, 0.25, format="%.2f",
                                help="Discounting, and the drift the "
                                     "risk-neutral price uses internally.")
        mu_pct = st.number_input(
            ...
            help="The stock's real-world drift. Drives the ℙ scenario "
                 "expectation and knock-out probability; by no-arbitrage "
                 "it never enters the risk-neutral price.")
...
st.markdown(f"""
<div class="cards">
  <div class="card hero"><div class="k">Risk-neutral price · ℚ</div>
    <div class="v">{res['price']:.4f}</div>
    <div class="s">95% CI <b>[{ci_lo:.4f}, {ci_hi:.4f}]</b> · arbitrage-free</div></div>
  <div class="card"><div class="k">Drift scenario · ℙ (μ={mu:.1%})</div>
...
**Market data — any ticker.** Type any listed symbol (AAPL, NVDA, TSLA…)
and the app pulls its live spot, ATM implied vol, and historical drift.
*Re-strike to spot* then scales the contract to that stock — K = spot,
B = K × 250/150 — preserving the original SPCX moneyness so prices stay
comparable across names. Spot is the latest yfinance trade (15–20 min
delayed).
